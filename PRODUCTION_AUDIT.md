...
日期：2026-07-05（原始审计） / 更新：2026-07-20（v1.3.1 后复审） / 更新：2026-07-30（v1.4.2 全面代码审查 + 安全加固） / 更新：2026-08-18（v1.5.0 发布审查） / 更新：2026-08-20（v1.5.1 发布审查）
...
| 版本 | 日期 | 审查类型 | 状态 |
|------|------|----------|------|
| v1.5.1 | 2026-08-20 | OpenCode Go 订阅额度与洞察/小组件发布门禁 | ✅ 本地验证完成，待 GitHub tag workflow 发布 |
...
| 指标 | 审计时 | v1.0.0 | v1.3.1 | v1.4.2 | v1.5.0 | v1.5.1 |
|------|--------|--------|--------|--------|--------|--------|
| 单元测试文件 | 16 | 22 | 47 | 53 | 86 shared + 2 variant | 86 shared + 2 variant |
| 测试数量 | 195 | 254+ | 700+ | 1,528 | 1,486 Debug + 1,486 Release | 1,531 Debug + 1,531 Release |
| Instrumented 测试 | 1 | 4 | 6 | 6 | 8 classes / 39 methods | 8 classes / 39 methods |
...
**v1.5.1 发布状态**：正式版本号为 v1.5.1。当前代码已通过本地 Debug/Release JVM、Lint、Kover 和 APK 构建门禁；发布提交、tag 和 GitHub Release 由 v1.5.1 tag workflow 管理。Release APK、测试门禁和最终 SHA-256 以 `TEST_REPORT.md` 与 GitHub Actions 记录为准。真实 Android 15/OEM 后台策略、精确闹钟配额和全量仪器测试发现仍是环境相关残余风险。

**v1.5.0 发布状态（历史）**：正式版本号为 v1.5.0。发布提交、tag 和 GitHub Release 由 tag workflow 管理；历史 Release APK、测试门禁和 SHA-256 记录在旧版审查材料中。
