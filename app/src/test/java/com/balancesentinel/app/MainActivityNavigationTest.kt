...
class MainActivityNavigationTest {
    ...
    @Test
    fun noLinkStartsHome() {
        assertEquals(AppRoute.Home.route, MainActivity.resolveStartDestination(Intent(), accounts, false))
    }

    @Test
    fun onboardingTakesPrecedence() {
    ...
    @Test
    fun canonicalValidLinkResolves() {
        val intent = Intent().setData(AppRoute.Insights("acct-1", "USD").toUri())
        assertEquals("insights/acct-1/USD", MainActivity.resolveStartDestination(intent, accounts, false))
    }

    @Test
    fun unknownAccountIsInvalid() {
        val intent = Intent().setData(Uri.parse("balancesentinel://insights/unknown/USD"))
        assertEquals(AppRoute.InvalidDeepLink("UnknownAccount").route, MainActivity.resolveStartDestination(intent, accounts, false))
    }

    @Test
    fun malformedPathIsInvalid() {
        val intent = Intent().setData(Uri.parse("balancesentinel://insights/acct-1/USD/extra"))
        assertEquals(AppRoute.InvalidDeepLink("MalformedUri").route, MainActivity.resolveStartDestination(intent, accounts, false))
    }

    @Test
    fun legacyIncompleteAndNonIsoCurrencyAreInvalid() {
    ...
    @Test
    fun childRoutesKeepTheirOwningTabSelected() {
